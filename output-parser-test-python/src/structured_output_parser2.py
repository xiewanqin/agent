...
JS: Zod + StructuredOutputParser.fromZodSchema
Python: Pydantic 模型 + PydanticOutputParser
"""
import json
import os
...
def main():
  ...
  try:
    ...
    for index, theory in enumerate(result.famous_theories):
      year_part = f" ({theory.year})" if theory.year is not None else ""
      print(f"   {index + 1}. {theory.name}{year_part}")
